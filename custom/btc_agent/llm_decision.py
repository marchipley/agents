...
def _build_system_prompt() -> str:
    return (
        ...
        "window_delta_pct is the source of truth for overall trend direction. velocity_30s is micro-momentum for entry timing only, not side selection.\n"
        ...
        "Respect market consensus. If the chosen side buy quote is below 0.15 and time_remaining_seconds is 15 or more, prefer NO_TRADE. Under 15 seconds, only fade consensus on a clear reversal. The same rule applies symmetrically for UP and DOWN.\n"
        "If window_delta_pct is positive and you choose DOWN, confidence must be below 0.50 unless trend exhaustion is clear. If window_delta_pct is negative and you choose UP, confidence must be below 0.50 unless trend exhaustion is clear.\n"
...
def _build_openai_realtime_system_prompt() -> str:
    return (
        ...
        "Use Window Delta as the strongest late signal. Window Delta only means change from market window open, not oracle_gap_ratio. window_delta_pct is the source of truth for overall trend direction; velocity_30s is entry timing only. "
        ...
        "Respect market consensus: if the chosen side quote is below 0.15 and time_remaining_seconds is 15 or more, prefer NO_TRADE; under 15 seconds only fade consensus on a clear reversal. "
        "If window_delta_pct is positive and you choose DOWN, confidence must stay below 0.50 unless trend exhaustion is clear; same symmetrically for UP in a negative window delta. "
...
def _build_user_prompt(features: BtcFeatures, market: BtcUpDownMarket, up_snapshot=None, down_snapshot=None) -> str:
    ...
    return (
        ...
        "- window_delta_pct is the source of truth for overall trend direction and side selection.\n"
        ...
        "- If the chosen side buy quote is below 0.15 and time_remaining_seconds is 15 or more, prefer NO_TRADE.\n"
        ...
        "- If window_delta_pct is positive and you choose DOWN, confidence must be below 0.50 unless trend exhaustion is clear. Apply the same rule symmetrically for UP in a negative window delta.\n"
...
def _build_compact_user_prompt(features: BtcFeatures, market: BtcUpDownMarket, up_snapshot=None, down_snapshot=None) -> str:
    ...
    return (
        ...
        "window_delta_pct is the source of truth for overall trend direction; velocity_30s is for entry timing only.\n"
        "Do not fade parabolic trend and do not chase if directional ticks are >= 8.\n"
        "If the chosen side quote is below 0.15 and time_remaining_seconds is 15 or more, prefer NO_TRADE.\n"
        ...
        "If window_delta_pct is positive and you choose DOWN, confidence must be below 0.50 unless exhaustion is clear; same symmetrically for UP in a negative window delta.\n"
...
def _build_minimal_user_prompt(features: BtcFeatures, market: BtcUpDownMarket, up_snapshot=None, down_snapshot=None) -> str:
    ...
    return (
        ...
        f"last10={features.last_10_ticks_direction}\n"
        ...
        "delta_pct chooses trend direction; v30 is entry timing only.\n"
        ...
        "If chosen side quote <0.15 and t>=15, prefer NO_TRADE; under 15 seconds only fade consensus on clear reversal.\n"
        ...
        "If delta_pct>0 and choosing DOWN, confidence must stay below 0.50 unless exhaustion is clear; same symmetrically for UP when delta_pct<0.\n"
...
def _build_openai_realtime_user_prompt(
    ...
    return (
        ...
        f"l10={features.last_10_ticks_direction};"
        "t_is_authoritative;"
        "if_t_gt_240_discovery_phase;"
        "d_is_trend_direction_v30_is_entry_timing;"
        "if_side_quote_lt_0.15_and_t_gte_15_prefer_no_trade;"
        ...
        "if_d_positive_and_choose_down_confidence_lt_0.50_unless_exhaustion;"
        "json only"
    )
